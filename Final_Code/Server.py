...
def log_in(username, password, c):
    """
    הפעולה הזאת אחראית על בדיקת המשתמש שרוצה להיכנס לעמוד התמונות. היא מקבל את שם המשתמש והסיסמא מהצד לקוח (FRON –
    END)  מהמשתמש והיא בודקת אם שם המשתמש והסיסמא נכונים (אם אין משתמש שכבר נכנס עם אותו שם משתמש וסיסמא והוא כרגע
    בפנים) אם הם נכונים היא שולחת לצד לקוח שהכל בסדר והוא מכניס את המשתמש לעמוד התמונות, אם הם שגויים היא מודיע על כך
    לצד לקוח, ישנם כמה מצבים של שגיאה: 1. השם משתמש או הסיסמא לא נמצאים במאגר נתונים. 2. יש כבר משתמש עם הנתונים האלה
    שהוא בפנים. :param username: שם משתמש :param password: סיסמא :param c: חיבור צפציפי עם הצד לקוח
    """
...
def client_back_to_start(c):
    """
    הפעולה הזאת אחראית כאשר המשתמש חוזר לעמוד הראשי בצד הלקוח. כאשר המשתמש חוזר לעמוד הראשי הצד לקוח מודיע זאת לצד
    שרת והצד שרת קורה לפעולה הזאת והיא מורידה את המשתמש שחזר לעמוד הראשי מרשימת המשתמשים שנמצאים כרגע בעמוד התמונות.
    :param c: חיבור צפציפי עם הצד לקוח
    """
    global userDict
